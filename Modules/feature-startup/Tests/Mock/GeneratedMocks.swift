...
public class MockStartupInteractor: StartupInteractor, Cuckoo.ProtocolMock {
    ...
    private var __defaultImplStub: StartupInteractor?

    public func enableDefaultImplementation(_ stub: StartupInteractor) {
        __defaultImplStub = stub
        cuckoo_manager.enableDefaultStubImplementation()
    }
    ...
    public struct __StubbingProxy_StartupInteractor: Cuckoo.StubbingProxy {
        ...
        func initialize<M1: Cuckoo.Matchable>(with splashAnimationDuration: M1) -> Cuckoo.ProtocolStubFunction<(TimeInterval), AppRoute> where M1.MatchedType == TimeInterval {
            ...
            return .init(stub: cuckoo_manager.createStub(for: MockStartupInteractor.self, method:
    """
    initialize(with: TimeInterval) async -> AppRoute
    """, parameterMatchers: matchers))
        }
