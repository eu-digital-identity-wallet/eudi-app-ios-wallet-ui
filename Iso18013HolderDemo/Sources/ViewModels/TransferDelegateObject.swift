...
class TransferDelegateObject: ObservableObject, MdocOfflineDelegate {
	...
	@Published var selectedRequestItems: IdentifiedArrayOf<DocElementsViewModel> = []
	...
	func didReceiveRequest(_ request: [String: Any], handleSelected: @escaping (Bool, RequestItems?) -> Void) {
		...
		selectedRequestItems = IdentifiedArray(uniqueElements: validRequestItems.map { docType,nsItems in DocElementsViewModel(docType: docType, elements: IdentifiedArray(uniqueElements: nsItems.map { (k,v) in v.map { ElementViewModel(nameSpace: k, elementIdentifier:$0)} }.flatMap {$0} )) })
